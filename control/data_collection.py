...
import socket
#import serial
from default_algorithm_backup import policy
    ...
    with conn:
        ...
        for i in range(100):
            ...
            for j in range(100):
                ...
                conn.sendall(data)
                new_x = np.array([[dt, puckCoordX, puckCoordY]])
                x = new_x if j == 0 else np.concatenate((x, new_x))
                new_y = np.array([[target_x_new, target_y_new, max_speed]])
                y = new_y if j == 0 else np.concatenate((y, new_y))

            with open('x_{}.npy'.format(i), 'wb') as f:
                    np.save(f, x)
            with open('y_{}.npy'.format(i), 'wb') as f:
                    np.save(f, y)
            #ser.write(data)
